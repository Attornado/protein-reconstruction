...
from models.classification.ugtransformer import UGFormerV2
            ...
            for h in grid_values['hidden_dim']:
                for n in grid_values['n_head']:
                    for lr in grid_values['learning_rate']:
                        ...
                        learning_rate = lr
                        if config['model_name'] == GAT_MODEL_TYPE:
                            for gn in grid_values['n_heads_gat']:
                                config['heads'] = gn
                                pscdb_baseline = UGFormerV2(dim_features=in_channels, dim_target=n_classes,
                                                            config=config)
                            ...
                            learning_rate = lr
                            pscdb_baseline = UGFormerV2(dim_features=in_channels, dim_target=n_classes, config=config)
